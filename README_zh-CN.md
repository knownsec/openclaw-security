...
OpenClaw 是一个可以运行在自己硬件上的强大的 AI 助手，于 2025 年 11 月首次发布，截至 2026 年 3 月已发布数十个版本。强大的同时也存在非常多的安全隐患，根据 ZoomEye 测绘数据，截至 2026 年 3 月 4 日，全球可识别的 OpenClaw 实例数量高达 63,026 个，GitHub Advisory Database 已收录相关漏洞多达 245 个，凸显其高速增长与安全挑战并存的现状，关于它的一些安全研究成果，[点击这里](docs/OpenClaw_Security_Analysis_2026_zh-CN.md)查看。

本项目是团队内部总结的 OpenClaw 全生命周期的安全实践指南，覆盖从安装、配置、日常使用、日常维护，助你在享受 OpenClaw 强大能力的同时守住安全底线。

# 快速开始

审计脚本支持 Linux、macOS 和 Windows。

## 第一步：评估当前环境安全

运行自动化审计脚本，快速检查 OpenClaw 环境的安全状况：
...
将 `OpenClaw安全实践指南.md` 复制到 OpenClaw 对话框中，让 AI 助手了解安全规范，在后续交互中遵循安全最佳实践。

## 第三步：学习日常安全技巧

通读本 README，掌握 OpenClaw 全生命周期的安全实践，包括安装配置、日常使用、应急响应等关键知识。
...
```
复制文件夹 %USERPROFILE%\.openclaw 到其他位置
```

## OpenClaw 配置检查

OpenClaw 配置文件位于 `~/.openclaw/openclaw.json`（Linux/macOS）或 `%USERPROFILE%\.openclaw\openclaw.json`（Windows）。
...
为了防止误操作，推荐给系统防火墙配置 18789 端口禁止访问的策略。
...
```
在 SKILL 目录打开搜索，查找关键字：exec、spawn、SECRET、KEY、TOKEN 等
```
...
```
打开命令提示符，输入：netstat -ano | findstr 18789
```
...
```
任务管理器 > 详细信息 > 查找 openclaw 进程，查看「用户名」
```
...
```
控制面板 > Windows Defender 防火墙 > 高级设置 > 入站规则 > 查找 18789
```
...
**建议将此脚本加入日常巡检，定期执行审计以持续保障 OpenClaw 安全。**
